...
async function main() {
  ...
  while (true) {
    ...
    convertedAmount = await apiService.useConvertionsCache(
      date,
    ...
    if (!convertedAmount)
      convertedAmount = await apiService.getResponse(
        date,
        baseCurrency.toUpperCase(),
        Number(amount),
        targetCurrency.toUpperCase()
    ...
    console.log(
      `${amount} ${baseCurrency.toUpperCase()} is ${convertedAmount} ${targetCurrency.toUpperCase()}`
    );
  }
}
